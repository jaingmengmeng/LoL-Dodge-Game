...
public class PlayerMove : MonoBehaviour
{
    ...
    void Update()
    {
        if (Input.GetMouseButtonDown(1))//当点击鼠标左键时（左键为0，右键为1）
        {
            ...
            {
                if (hit.collider.gameObject.name == "Plane")//当射线碰撞到的是Plane（此if语句限制鼠标点击位置在Plane上有效）
                {
                    ...
                    MoveSpeed = FastSpeed;//速度赋为10,
                }
            }
        }
        if (Vector3.Distance(transform.position, hit.point) < 0.1f)//到达目标地址，自身位置和鼠标点击位置小于0时，近似于停止
        {
